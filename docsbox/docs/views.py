...
from docsbox.settings import MEDIA_PATH
    ...
    def post(self):
        ...
        if "file" not in request.files:
            ...
            with NamedTemporaryFile(
                delete=False, prefix=app.config["MEDIA_PATH"]
            ) as tmp_file:
                ...
                if mimetype not in app.config["SUPPORTED_MIMETYPES"]:
                    return abort(
                        400, message="Not supported mimetype: '{0}'".format(mimetype)
                    )
                ...
                if options:  # options validation
                    ...
                    if not isinstance(formats, list) or not formats:
                        ...
                        for fmt in formats:
                            supported = (
                                fmt
                                in app.config["SUPPORTED_MIMETYPES"][mimetype][
                                    "formats"
                                ]
                            )
                            if not supported:
                    ...
                    if thumbnails:
                        if not isinstance(thumbnails, dict):
                            ...
                            if (
                                not isinstance(thumbnails_size, str)
                                or not thumbnails_size
                            ):
